...
def CreateAcc(id:String,no:Int, b:Double):Unit={
 ...
 CreateAcc("10001",202201,500)
 CreateAcc("10002",202202,-10)
 println(bank)
 ...
 val totBalWithInterest = (b:List[Account]) => b.map(account => if(account.balance > 0) account.balance + account.balance*0.05 else account.balance*0.1)
 ...
 println(totBalWithInterest(bank))
